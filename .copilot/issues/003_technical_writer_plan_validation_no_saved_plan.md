...
2. **No possibility to review the plan manually**: Users cannot inspect what the agent actually produced
3. **Missing trace events**: No detailed logs/events in journal showing the plan content or validation failure details
...
2. **Detailed error logging**: Log the specific validation errors and the raw plan content
3. **Manual review capability**: Allow users to inspect failed plans via `exoctl plan show <id>` or similar
...
4. **Error Details**: Provide specific validation error messages in request status</content>
<parameter name="filePath">/home/dkasymov/git/ExoFrame/.agents/issues/003_technical_writer_plan_validation_no_saved_plan.md
