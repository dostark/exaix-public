...
The Exaix daemon ignores `EXA_LLM_PROVIDER` and `EXA_LLM_MODEL` environment variables when selecting which AI provider to use for request processing. Despite these variables being correctly set in the daemon's environment, the provider selection logic always falls back to the mock provider due to preferring "free" providers.
...
export EXA_LLM_MODEL=gemini-2.0-flash-exp

# 2. Start daemon with these variables
exactl daemon start

# 3. Verify variables are set in daemon process
cat /proc/$(pgrep -f "exactl daemon")/environ | tr '\0' '\n' | grep EXA_LLM
# Shows: EXA_LLM_PROVIDER=google, EXA_LLM_MODEL=gemini-2.0-flash-exp

# 4. Create a request
exactl request "Test request"

# 5. Check journal - provider selected is still "mock"
exactl journal --tail 5
```

## Observed Behavior

- Environment variables `EXA_LLM_PROVIDER` and `EXA_LLM_MODEL` are correctly inherited by the daemon process
...
- When `EXA_LLM_PROVIDER` is set, that provider should be used
- When `EXA_LLM_MODEL` is set, that model should be used
...
- OS: Linux
- Deno Version: 1.x.x
- Relevant Config: `prefer_free = false` in exa.config.toml (tested both true and false)

## Root Cause Analysis

**Primary Issue:** Environment variables `EXA_LLM_PROVIDER` and `EXA_LLM_MODEL` are validated but never applied to override configuration.

**Technical Details:**

1. **Validation exists but is unused:** The `getValidatedEnvOverrides()` function in `src/config/env_schema.ts` correctly validates EXA_LLM_* environment variables but is never called.
...
- [x] EXA_LLM_* variables are validated correctly
...
- [x] Need to modify selection to respect EXA_LLM_PROVIDER override

### Test Mode Impact

- [x] EXA_TEST_MODE not set (confirmed via `env | grep EXO`)
- [x] Test mode settings commented out in exa.config.toml
...
1. **Modified ProviderSelector.selectProviderForTask()** in `src/ai/provider_selector.ts`:
   ...
   - If `EXA_LLM_PROVIDER` is set and the provider is registered and healthy, return it immediately
   - **Safety Check:** Blocks paid providers in test/CI environments unless `EXA_TEST_ENABLE_PAID_LLM=1` is set
...
- **Safety Check:** Paid providers are blocked in test/CI without `EXA_TEST_ENABLE_PAID_LLM=1`
- **Integration Test Confirmed:** Daemon now respects `EXA_LLM_PROVIDER=google` and `EXA_LLM_MODEL=gemini-2.0-flash-exp`
...
- **Safety:** Paid providers blocked in test/CI without explicit opt-in via `EXA_TEST_ENABLE_PAID_LLM=1`
